...
#include "../../../include/linux/iio/iio-gts-helper.h"
#else
				      ...
				      enum iio_event_direction dir)
{
	...
	case IIO_EV_TYPE_THRESH: {
		...
		if (chan->type == IIO_LIGHT) {
			ret = int_en && (int_src == APDS9306_INT_SRC_ALS);
			break;
		}

		if (chan->type == IIO_INTENSITY) {
			ret = int_en && (int_src == APDS9306_INT_SRC_CLEAR);
			break;
		}
				       ...
				       int state)
{
	...
	case IIO_EV_TYPE_THRESH_ADAPTIVE:
		if (state)
			ret = regmap_field_write(rf->int_thresh_var_en, 1);
		else
...
#endif
{
	...
	if (IS_ERR(data->vdd))
		return dev_err_probe(dev, IS_ERR(data->vdd),
...
static SIMPLE_DEV_PM_OPS(apds9306_pm_ops, apds9306_runtime_suspend,
